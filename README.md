...
Бэйдж — кликабельный (OSC 8): `Ctrl/Cmd+click` открывает каталог `docs/backlogs`
текущего репозитория. Ссылка использует `file://`, поэтому клик срабатывает там, где
путь к проекту физически доступен (локальный терминал). Терминалы без поддержки
OSC 8 просто показывают `📋 N` без ссылки.
