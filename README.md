...
1. **Capture** — Claude **никогда не пишет в бэклог молча сам по себе**. Запись
   происходит ровно в трёх случаях: **(1)** юзер явно попросил («положи в бэклог /
   запиши на потом»); **(2)** Claude по ходу работы заметил НЕсвязанный с задачей
   баг или побочную задачу вне scope — тогда он **спрашивает** «записать в бэклог?»
   и пишет только после согласия; **(3)** фоновая/автономная сессия (circle /
   worktree / loop), где задача требует решения юзера, а спросить некого — тогда
   пишет, чтобы решение не потерялось. Задачи внутри scope открытого плана остаются
   в его трекинге, а не в бэклоге.
...
Файлы `docs/backlogs/` — это обычные tracked-артефакты проекта (метаданные задачи

- контекст, без секретов), их **коммитят по умолчанию**, а не выкидывают как
  посторонние правки. В `.gitignore` папку добавлять не нужно.
